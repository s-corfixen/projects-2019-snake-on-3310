...
class NokiaSnakeClient(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #We set title for the app frame
...
class PageOne(tk.Frame):
    def __init__(self, parent, controller):
        ...
        #The second runs the function clearall defined in the class NokiaSnakeClient
...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        #we define the function getdataset. The function retrives the data set specified by the checkbox lists on the page.
        def getdataset():
            selectedvariables = {}
        ...
        #The generate function generates several interior frames on the page, which contains lists of checkboxes.
...
#We define the class graphwindow, which creates a window with graphs. 
#The class is not implemented in the app itself yet. You can find similiraties between the class and the makegraph()
#function on PageThree
class graphwindow(tk.Tk):
    def __init__(self, data, x, y, y2, graphnames, *args, **kwargs):
        tk.Tk.__init__(self, *args, **kwargs)
        
        #we create a figure and add subplots
        fig = Figure()
        addsubplot = fig.add_subplot(1,1,1)
        #addsubplot2 uses the secondary y-axis.
        addsubplot2 = addsubplot.twinx()
        
        #We create a figure canvas for our figure
        canvas = FigureCanvasTkAgg(fig, self)
        canvas.get_tk_widget().pack(side=tk.BOTTOM,fill=tk.BOTH, expand = True)

        #we place a graphtoolbar in the window
        ...
        #The graph menu have a dropdown menu to choose between different plots.
        #We retrieve graphnames
        graphs = sorted(data[graphnames].unique())

        #we create a string variable and set its default value
        graphmenuvariable = tk.StringVar()
        ...
        #we define the function creategraph, which is referenced by the updategraph button below
        def creategraph(data, x, y, y2):
            
            #we clear the subplots
            addsubplot.clear()
            addsubplot2.clear()
            ...
            #show x axis from primary y axis
            addsubplot.axhline(y=0, color='k')
            
            #We draw the plots on the canvas
            canvas.draw()
...
if __name__ == "__main__":
    ...
    #we set the NokiaSnakeClient class as our app. We adjust the default size 
